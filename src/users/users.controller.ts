...
export class UsersController {
  ...
  // ADMIN: Ver todos los ciudadanos
  @Get('ciudadanos')
  @Roles(UserType.ADMIN, UserType.ENTIDAD)
  async getAllCiudadanos() {
    const ciudadanos = await this.usersService.getAllCiudadanos();
    return ciudadanos.map(({ contraseña, ...c }) => c);
  }

  // ADMIN: Ver todas las entidades
  @Get('entidades')
  @Roles(UserType.ADMIN)
  async getAllEntidades() {
    const entidades = await this.usersService.getAllEntidades();
    return entidades.map(({ contraseña, ...e }) => e);
  }

  // ADMIN: Ver todos los administradores
  @Get('administradores')
  @Roles(UserType.ADMIN)
  async getAllAdmins() {
    const admins = await this.usersService.getAllAdmins();
    return admins.map(({ contraseña, ...a }) => a);
  }
}
